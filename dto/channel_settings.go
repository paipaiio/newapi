...
type ChannelSettings struct {
	...
	CostRatio              float64 `json:"cost_ratio,omitempty"` // 成本系数：成本 = 该渠道标准原价(logs.quota÷分组倍率) × cost_ratio
	Rpm                    int     `json:"rpm,omitempty"`        // 该渠道每分钟最大请求数，0=不限
	Tpm                    int     `json:"tpm,omitempty"`        // 该渠道每分钟最大 token 数，0=不限
}
