...
func chooseDB(envName string, isLog bool) (*gorm.DB, common.DatabaseType, error) {
	...
	if dsn != "" {
		...
		// 确保连接使用 utf8mb4，避免中文等多字节字符写入报 Incorrect string value
...
func InitLogDB() (err error) {
	if os.Getenv("LOG_SQL_DSN") == "" {
		...
		initCol()
		return migrateLOGDB() // 无独立日志库时也执行迁移，确保 session/conversation 表结构同步
	}
...
func migrateLOGDB() error {
	...
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		// ClickHouse 不支持这些 GORM AutoMigrate；session/monitor 表要求关系型 LOG_DB
		return migrateClickHouseLogDB()
	}
	if err := LOG_DB.AutoMigrate(&Log{}); err != nil {
		return err
	}
	if err := LOG_DB.AutoMigrate(&SessionLog{}); err != nil {
		return err
	}
