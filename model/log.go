...
// ChannelGroupStat 按渠道+分组聚合的日志统计行。
type ChannelGroupStat struct {
	ChannelId int    `json:"channel_id"`
	Group     string `json:"group"`
...
func GetChannelGroupStats(startTimestamp, endTimestamp int64) ([]ChannelGroupStat, error) {
	var rows []ChannelGroupStat
	tx := LOG_DB.Table("logs").
		Select("channel_id, `group`, sum(quota) quota, sum(prompt_tokens)+sum(completion_tokens) tokens, count(*) count").
		Where("type = ?", LogTypeConsume)
	if startTimestamp != 0 {
		tx = tx.Where("created_at >= ?", startTimestamp)
	}
	...
	err := tx.Group("channel_id, `group`").Scan(&rows).Error
	return rows, err
}

// DailyChannelGroupStat 按天 + (channel_id, group) 聚合的统计行。
type DailyChannelGroupStat struct {
	Day       int64  `json:"day"` // created_at / 86400，即从 epoch 起的天数
	ChannelId int    `json:"channel_id"`
	Group     string `json:"group"`
	Quota     int64  `json:"quota"`
	Tokens    int64  `json:"tokens"`
...
// GetDailyChannelGroupStats 按天 + (channel_id, group) 聚合消费日志，用于利润趋势报表。
// day = created_at / 86400（UTC 天），前端按需转本地日期。
func GetDailyChannelGroupStats(startTimestamp, endTimestamp int64) ([]DailyChannelGroupStat, error) {
	...
	err := tx.Group("day, channel_id, `group`").Order("day asc").Scan(&rows).Error
	return rows, err
}
