...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	// TPM 事后累计：把本次实际消耗的 token 计入用户/令牌/渠道三级窗口
	totalTokens := summary.PromptTokens + summary.CompletionTokens
	if totalTokens > 0 {
		common.AddTPM("user", relayInfo.UserId, totalTokens)
		common.AddTPM("token", relayInfo.TokenId, totalTokens)
		common.AddTPM("channel", relayInfo.ChannelId, totalTokens)
	}

	// 会话正文捕获(异步、可配置、失败不影响主链路)。
	CaptureSession(ctx, relayInfo, SessionCaptureMeta{
