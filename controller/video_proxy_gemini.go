...
func getGeminiVideoURL(channel *model.Channel, task *model.Task, apiKey string) (string, error) {
	...
	resp, err := adaptor.FetchTask(baseURL, apiKey, &model.Task{
	...
	taskInfo, parseErr := adaptor.ParseTaskResult(task, nil, body)
...
func getVertexVideoURL(channel *model.Channel, task *model.Task) (string, error) {
	...
	resp, err := adaptor.FetchTask(baseURL, key, &model.Task{
		TaskID: task.GetUpstreamTaskID(),
		Action: task.Action,
