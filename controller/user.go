...
func setupLoginAtAuthVersion(user *model.User, expectedAuthVersion int64, c *gin.Context) {
	...
	if currentUser.Role != common.RoleRootUser && currentUser.LoginDisabled {
...
func SearchUsers(c *gin.Context) {
	...
	pageInfo := common.GetPageQuery(c)
	sortOptions := model.NewUserSortOptions(c.Query("sort_by"), c.Query("sort_order"))
	users, total, err := model.SearchUsersWithQuota(keyword, group, role, status, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), quotaMin, quotaMax, sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func ManageUser(c *gin.Context) {
	...
	case "disable_login":
		...
		_ = model.InvalidateUserCache(user.Id)
		_, _ = model.RevokeAllUserSessions(user.Id, "admin_disable_login")
