...
func setupLogin(user *model.User, c *gin.Context) {
	// 禁止登录拦截：login_disabled 的用户不能登录 Web 控制台（其 API key 仍可正常调用）。
	// setupLogin 是所有登录方式（密码/2FA/oauth/wechat/passkey/telegram）的统一汇合点；
	// 第三方登录传入的 user 对象可能未填 login_disabled，按 Id 重查一次确保准确。root 用户豁免，避免误锁。
	if user.Role != common.RoleRootUser {
		if fresh, err := model.GetUserById(user.Id, false); err == nil && fresh != nil && fresh.LoginDisabled {
			c.JSON(http.StatusOK, gin.H{
				"message": "该账号已被禁止登录控制台，但 API 调用不受影响。如有疑问请联系管理员。",
				"success": false,
			})
			return
		}
...
func SearchUsers(c *gin.Context) {
	...
	pageInfo := common.GetPageQuery(c)
	users, total, err := model.SearchUsers(keyword, group, role, status, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), quotaMin, quotaMax)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func GetUserModels(c *gin.Context) {
	...
	groups := service.GetUserUsableGroupsByUser(user.Id, user.Group)
	group := c.Query("group")
	if group != "" {
...
func ManageUser(c *gin.Context) {
	...
	case "add_quota":
		...
		return
	case "allow_topup":
		if err := model.DB.Model(&model.User{}).Where("id = ?", user.Id).Update("allow_topup", true).Error; err != nil {
			common.ApiError(c, err)
			return
		}
		...
		return
	case "disallow_topup":
		if err := model.DB.Model(&model.User{}).Where("id = ?", user.Id).Update("allow_topup", false).Error; err != nil {
			common.ApiError(c, err)
			return
		}
		_ = model.InvalidateUserCache(user.Id)
	...
	case "disable_login":
		...
		if err := model.DB.Model(&model.User{}).Where("id = ?", user.Id).Update("login_disabled", true).Error; err != nil {
			common.ApiError(c, err)
			return
		}
		_ = model.InvalidateUserCache(user.Id)
		recordManageAuditFor(c, user.Id, "user.disable_login", map[string]interface{}{"username": user.Username, "id": user.Id})
		c.JSON(http.StatusOK, gin.H{"success": true, "message": ""})
		return
	case "enable_login":
		if err := model.DB.Model(&model.User{}).Where("id = ?", user.Id).Update("login_disabled", false).Error; err != nil {
			common.ApiError(c, err)
			return
		}
		_ = model.InvalidateUserCache(user.Id)
...
type UpdateUserSettingRequest struct {
	...
	RedactRequestEnabled             bool    `json:"redact_request_enabled"`
}
...
func UpdateUserSetting(c *gin.Context) {
	...
	settings := dto.UserSetting{
		...
		RedactRequestEnabled:             req.RedactRequestEnabled,
	}
...
type BatchTopupRequest struct {
	IDs        []int `json:"ids"`
	AllowTopup bool  `json:"allow_topup"`
}

func BatchManageTopup(c *gin.Context) {
	var req BatchTopupRequest
	if err := c.ShouldBindJSON(&req); err != nil || len(req.IDs) == 0 || len(req.IDs) > 500 {
		common.ApiErrorI18n(c, i18n.MsgInvalidParams)
		return
	}
	if err := model.DB.Model(&model.User{}).Where("id IN ?", req.IDs).Update("allow_topup", req.AllowTopup).Error; err != nil {
		common.ApiError(c, err)
		return
	}
