...
export function BurnTool() {
  ...
  return (
    <SectionPageLayout
      header={{
        title: '额度验货 / 消耗工具',
        description: '真实发请求测试 API Key 的可用模型与实际额度',
        extra: running ? (
        ...
        <Card>
          ...
          <CardContent className='space-y-4'>
            ...
            <div className='grid grid-cols-2 gap-3'>
              ...
              <div className='space-y-1.5'>
                <Label>{stopType === 'spend' ? '目标金额 ($)' : stopType === 'requests' ? '请求数' : stopType === 'duration' ? '秒数' : '（无需填）'}</Label>
        ...
        <div className='space-y-4'>
          ...
          {stats.start_usd > 0 && (
            <div className='text-xs text-muted-foreground'>
              起始余额 ${stats.start_usd.toFixed(2)} → 当前 ${stats.remain_usd.toFixed(2)}
              （实测消耗 ${stats.spent_usd.toFixed(4)}）
            </div>
          )}
          <Card>
