...
import {
  getGroupUsage,
  type GroupUsageStats,
} from "@/features/group-management/api";
...
export const GroupRatioVisualEditor = memo(function GroupRatioVisualEditor({
  ...
  const handleAutoGroupAdd = useCallback(
    (name: string) => {
      ...
      onChange(
        "AutoGroups",
        JSON.stringify([...autoGroupsList, name], null, 2),
      );
    },
...
function GroupPricingTable({
  ...
  // Delete safety: warn when the group is still referenced by
  // users / tokens / channels before removing it locally.
  ...
  const handleDeleteClick = useCallback(
    async (row: GroupPricingRow) => {
      ...
      try {
        const res = await getGroupUsage(name);
        const usage = res.success ? res.data : undefined;
        if (usage && usage.users + usage.tokens + usage.channels > 0) {
          setPendingDelete({ row, usage });
          return;
        }
      } catch {
        /* usage stats unavailable — fall through to plain removal */
      }
      removeRow(row._id);
    },
    [removeRow],
  ...
  return (
    <Card className={sectionCardClassName}>
      ...
      <ConfirmDialog
        ...
        desc={t(
          "This group is still referenced by {{users}} users, {{tokens}} tokens and {{channels}} channels. Deleting it breaks their billing until reconfigured.",
          {
            users: pendingDelete?.usage.users ?? 0,
            tokens: pendingDelete?.usage.tokens ?? 0,
            channels: pendingDelete?.usage.channels ?? 0,
          },
        )}
        destructive
        handleConfirm={() => {
          if (pendingDelete) removeRow(pendingDelete.row._id);
...
function GroupDetailSheet(props: GroupDetailSheetProps) {
  ...
  // Live reference counts (users / tokens / channels) for delete safety.
  const { data: usage } = useQuery({
    queryKey: ["group-usage", name],
    queryFn: async () => {
      const res = await getGroupUsage(name!);
              ...
              <dl className="space-y-1.5 text-sm">
                ...
                {usage && (
                  <div className="flex justify-between gap-4">
                    ...
                    <dd className="text-right font-medium">
                      {t("{{users}} users / {{tokens}} tokens / {{channels}} channels", {
                        users: usage.users,
                        tokens: usage.tokens,
