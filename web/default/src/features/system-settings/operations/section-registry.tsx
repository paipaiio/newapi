...
const OPERATIONS_SECTIONS = [
  ...
  {
    id: 'storage',
    titleKey: 'Session Storage (R2)',
    build: (settings: OperationsSettings) => (
      <StorageSettingsSection
        defaultValues={{
          'storage_setting.enabled':
            settings['storage_setting.enabled'] ?? false,
          'storage_setting.capture_failed':
            settings['storage_setting.capture_failed'] ?? false,
          'storage_setting.endpoint':
            settings['storage_setting.endpoint'] ?? '',
          'storage_setting.region':
            settings['storage_setting.region'] ?? 'auto',
          'storage_setting.bucket': settings['storage_setting.bucket'] ?? '',
          'storage_setting.access_key':
            settings['storage_setting.access_key'] ?? '',
          'storage_setting.secret_key':
            settings['storage_setting.secret_key'] ?? '',
          'storage_setting.key_prefix':
            settings['storage_setting.key_prefix'] ?? 'conv',
          'storage_setting.retention_days':
            settings['storage_setting.retention_days'] ?? 0,
          'storage_setting.max_body_bytes':
            settings['storage_setting.max_body_bytes'] ?? 1048576,
        }}
      />
    ),
  },
