...
export function Hero(props: HeroProps) {
  ...
  const newUserReward = fmtQuota(status?.quota_for_new_user as number | undefined);
  const inviterReward = fmtQuota(status?.quota_for_inviter as number | undefined);
            ...
            >
              {newUserReward && (
                <span className="border-border/40 bg-muted/20 text-muted-foreground/90 inline-flex items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-xs font-medium backdrop-blur-xs">
                  ...
                  {t("Sign up for {{amount}} credits", { amount: newUserReward })}
                </span>
              )}
              {inviterReward && (
                <span className="border-border/40 bg-muted/20 text-muted-foreground/90 inline-flex items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-xs font-medium backdrop-blur-xs">
