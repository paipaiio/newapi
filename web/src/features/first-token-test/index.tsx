  ...
  return (
    <Card>
      <CardContent className='p-3'>
        ...
        <div
          className={`mt-0.5 text-lg font-bold tabular-nums ${valueClass ?? ''}`}
        >
          {value}
        </div>
      </CardContent>
            ...
            try {
              ...
              if (evt.type === 'error') {
                ...
                if (evt.status === 'success') {
                  pushLog(
                    `#${evt.index} ✓ 首字 ${fmtMs(evt.first_ms)} · ${evt.first_token ?? ''}`
                  )
                } else {
                ...
                setRunning(false)
                pushLog(
                  `✅ 完成：${evt.ok}/${evt.sent} 成功，平均 ${fmtMs(evt.avg_ms)}，P95 ${fmtMs(evt.p95_ms)}，标准差 ${fmtMs(evt.stddev_ms)}`
                )
              }
            } catch {
    ...
    <div className='grid gap-6 lg:grid-cols-2'>
      ...
      <div className='space-y-4'>
        <div className='grid grid-cols-2 gap-3'>
          ...
          <MiniStat
            label='平均首字'
            value={fmtMs(stats.avg_ms)}
            valueClass='text-yellow-500'
          />
          <MiniStat
            label='P50 / P95'
            value={`${fmtMs(stats.p50_ms)} / ${fmtMs(stats.p95_ms)}`}
          />
          <MiniStat
            label='最小 / 最大'
            value={`${fmtMs(stats.min_ms)} / ${fmtMs(stats.max_ms)}`}
          />
        ...
        <Card>
          ...
          <CardContent>
            <div
              ref={logBoxRef}
              className='bg-muted/50 h-[220px] overflow-y-auto rounded-md p-3 font-mono text-xs leading-relaxed'
            >
              {logs.length === 0 ? (
            ...
            <div className='max-h-[260px] overflow-y-auto rounded-md border text-xs'>
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <div
                    key={row.index}
                    className='grid grid-cols-[56px_72px_96px_1fr] gap-2 border-b px-2 py-1.5 last:border-b-0'
                  >
