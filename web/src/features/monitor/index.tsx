            ...
            for (const line of block.split('\n')) {
              ...
              try {
                ...
                if (prev && data.ts > prev.ts) {
                  rps = Math.max(
                    0,
                    (data.req_last_5min - prev.req_last_5min) /
                      (data.ts - prev.ts)
                  )
                }
                ...
                setHistory((h) => {
                  const next = [
                    ...h,
                    { time: t, rps: parseFloat(rps.toFixed(2)) },
                  ]
  ...
  return (
    <div className='space-y-4'>
      <div className='flex items-center justify-between'>
        ...
        >
          <span
            className={`inline-block h-2 w-2 rounded-full ${connected ? 'animate-pulse bg-green-400' : 'bg-red-400'}`}
          />
      ...
      <div className='grid grid-cols-2 gap-4 sm:grid-cols-4'>
        ...
        <StatCard
          ...
          sub={
            stats
              ? `近 1 小时 ${stats.req_last_1h.toLocaleString()} 次`
              : undefined
          }
          icon={Activity}
      ...
      <Card>
        <CardHeader>
          <CardTitle className='text-sm'>请求速率 (req/s) — 实时推送</CardTitle>
        </CardHeader>
        <CardContent>
