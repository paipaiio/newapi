...
function BurnToolContent() {
  ...
  const start = async () => {
    ...
    setLogs([])
    pushLog(
      `开始：模型 ${model}，并发 ${concurrency}，停止条件 ${stopType}=${stopType === 'until_empty' ? '耗尽' : stopValue}`
    )
          ...
          for (const line of block.split('\n')) {
            ...
            try {
              ...
              if (evt.type === 'error') {
                ...
                setRunning(false)
                pushLog(
                  `✅ 完成：发送 ${evt.sent}（ok ${evt.ok}/fail ${evt.fail}），实测消耗 $${evt.spent_usd.toFixed(4)}，耗时 ${evt.elapsed.toFixed(1)}s`
                )
              }
            } catch {
              /* ignore */
    ...
    <div className='grid gap-6 lg:grid-cols-2'>
        ...
        <CardContent className='space-y-4'>
          ...
          <div className='grid grid-cols-2 gap-3'>
            <div className='space-y-1.5'>
              ...
              <Select
                value={stopType}
                onValueChange={(v) => setStopType(v ?? 'spend')}
                disabled={running}
              >
                <SelectTrigger>
                  <SelectValue />
      ...
      <div className='space-y-4'>
        <div className='grid grid-cols-2 gap-3'>
          ...
          <MiniStat
            label='实测消耗'
            value={`$${stats.spent_usd.toFixed(4)}`}
            valueClass='text-orange-500'
          />
          <MiniStat
            label='当前余额'
            value={
              stats.remain_usd > 0 ? `$${stats.remain_usd.toFixed(2)}` : '—'
            }
          />
        </div>
        {stats.start_usd > 0 && (
          <div className='text-muted-foreground px-1 text-xs'>
            起始余额 ${stats.start_usd.toFixed(2)} → 当前 $
            {stats.remain_usd.toFixed(2)}（实测消耗 $
            {stats.spent_usd.toFixed(4)}）
          </div>
        )}
        <Card>
          ...
          <CardContent>
            <div
              ref={logBoxRef}
              className='bg-muted/50 h-[280px] overflow-y-auto rounded-md p-3 font-mono text-xs leading-relaxed'
            >
              {logs.length === 0 ? (
