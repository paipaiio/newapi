...
function daysAgoRange(days: number) {
  const now = Date.now()
  return {
    start: Math.floor((now - days * 86400_000) / 1000),
...
export function RequestStats() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      isAdmin ? getSuccessStats(params) : getSelfSuccessStats(params),
    staleTime: 60_000,
  })
  ...
  const filteredUsers = useMemo(() => {
    ...
    return q
      ? stats.by_user.filter((u) => u.username.toLowerCase().includes(q))
      : stats.by_user
      ...
      {isLoading ? (
        ...
        <div className='grid grid-cols-2 gap-3 sm:grid-cols-4'>
          <StatCard
            ...
            sub={t('Avg time: {{t}}', {
              t: fmtTime(stats?.summary.avg_use_time ?? 0),
            })}
          />
          <StatCard
            title={t('Successful')}
          ...
          <StatCard
            ...
            sub={fmtRate(
              stats?.summary.failed != null && stats?.summary.total > 0
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className='w-24 text-right text-green-600'>
                        {t('Success')}
                      </TableHead>
                      <TableHead className='w-24 text-right text-red-500'>
                        {t('Failed')}
                      </TableHead>
