...
export function OAuthConsentPage({ requestId }: { requestId: string }) {
  ...
  const { data: info, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (res.data?.auto_approve) {
        const approval = await postOAuthConsent(requestId, 'approve')
        if (approval.success && approval.data?.redirect) {
          window.location.href = approval.data.redirect
          return null
        }
        toast.error(approval.message || t('Operation failed'))
        return null
      }
