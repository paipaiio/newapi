...
const renderStatistics = (text, record, showEnableDisableModal, t) => {
  ...
  const content = (
    <span className='inline-flex items-center gap-1'>
      ...
      {!isDeleted && record.login_disabled ? (
...
// 渲染「可见分组」列：解析 record.setting.visible_groups
// 空 => 不限制（看到全部）；非空 => 展示受限的分组标签
const renderVisibleGroups = (text, record, t) => {
  let vg = [];
  try {
    if (record && record.setting) {
      const st = JSON.parse(record.setting);
      if (Array.isArray(st.visible_groups)) vg = st.visible_groups;
    }
  } catch (e) {}
  if (!vg || vg.length === 0) {
    return (
  ...
  return (
    <Space spacing={2} wrap>
      {vg.map((g) => (
        <Tag key={g} color='violet' shape='circle' size='small'>
          {g}
        </Tag>
      ))}
    </Space>
...
const renderOperations = (
  ...
  const moreMenu = [
    {
      node: 'item',
      name: record.allow_topup === false ? t('开启充值') : t('禁止充值'),
      onClick: () => toggleTopup(record),
    },
    {
      node: 'item',
      name: t('订阅管理'),
    ...
    {
      node: 'item',
      name: record.login_disabled ? t('允许登录') : t('禁止登录'),
      onClick: () =>
        manageUser(
          record.id,
          record.login_disabled ? 'enable_login' : 'disable_login',
          record,
        ),
    },
...
export const getUsersColumns = ({
  ...
  return [
    ...
    {
      title: t('可见分组'),
      dataIndex: 'visible_groups',
      render: (text, record, index) => renderVisibleGroups(text, record, t),
    },
    {
      title: t('角色'),
      dataIndex: 'role',
