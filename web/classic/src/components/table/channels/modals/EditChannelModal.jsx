...
const EditChannelModal = (props) => {
    ...
    if (success) {
      ...
      if (data.setting) {
        try {
          ...
          data.cost_ratio = parsedSettings.cost_ratio || 0;
          data.rpm = parsedSettings.rpm || 0;
          data.tpm = parsedSettings.tpm || 0;
        } catch (error) {
  ...
  const submit = async () => {
    ...
    const channelExtraSettings = {
      ...
      cost_ratio: localInputs.cost_ratio || 0,
      rpm: localInputs.rpm || 0,
      tpm: localInputs.tpm || 0,
    };
            ...
            const advancedSettingsContent = (
              <div className='space-y-4'>
                ...
                <div className='pt-3'>
                  ...
                  <Form.InputNumber field='cost_ratio' label={t('成本系数')} placeholder='0' min={0} step={0.01} onChange={(value) => handleChannelSettingsChange('cost_ratio', value ?? 0)} extraText={t('成本 = 该渠道标准原价 × 成本系数（标准原价 = 售价quota ÷ 分组倍率）。0 表示未配置，利润按未知处理')} />
                  <Form.InputNumber field='rpm' label={t('渠道 RPM 限制')} placeholder='0' min={0} step={1} onChange={(value) => handleChannelSettingsChange('rpm', value ?? 0)} extraText={t('该渠道每分钟最大请求数，超限后自动重试其他渠道。0=不限')} />
                  <Form.InputNumber field='tpm' label={t('渠道 TPM 限制')} placeholder='0' min={0} step={1} onChange={(value) => handleChannelSettingsChange('tpm', value ?? 0)} extraText={t('该渠道每分钟最大 token 数（事后累计）。0=不限')} />
                </div>
              </div>
            );
