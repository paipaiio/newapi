...
const EditTokenModal = (props) => {
  ...
  const submit = async (values) => {
    setLoading(true);
    // 分组多选:数组 → 逗号分隔字符串(保持顺序=优先级)。兼容已是字符串的情况。
                ...
                <Row gutter={12}>
                  ...
                  <Col span={24}>
                    {groups.length > 0 ? (
                      <Form.Select
                        field='group'
                        label={t('令牌分组')}
                        placeholder={t('可多选分组，按选择顺序作为优先级，请求时依次查找有可用渠道的分组')}
                        optionList={groups}
                  ...
                  <Col xs={24} sm={12} md={12} lg={7} xl={7}>
                    <Form.InputNumber
                      ...
                      step={1}
                      style={{ width: '100%' }}
                      extraText={t('该密钥每分钟最大 token 数，0=不限')}
                    />
                  </Col>
