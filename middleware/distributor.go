		...
		if ok {
			...
			if shouldSelectChannel {
				...
				if strings.HasPrefix(c.Request.URL.Path, "/pg/") {
					playgroundGroup := playgroundGroupFromRequest(c)
					if playgroundGroup != "" {
...
func playgroundGroupFromRequest(c *gin.Context) string {
	contentType := c.ContentType()
	if slices.Contains([]string{gin.MIMEPOSTForm, gin.MIMEMultipartPOSTForm}, contentType) {
		return strings.TrimSpace(c.PostForm("group"))
	}
	playgroundRequest := &dto.PlayGroundRequest{}
	if err := common.UnmarshalBodyReusable(c, playgroundRequest); err != nil {
		return ""
	}
	return strings.TrimSpace(playgroundRequest.Group)
}
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.NewAPIError {
	...
	// 渠道级限流（RPM/TPM）：超额时返回可重试错误，让上层切换到其他渠道
	chSetting := channel.GetSetting()
	if !common.CheckTPM("channel", channel.Id, chSetting.Tpm) {
		return types.NewError(errors.New("channel TPM limit exceeded"), types.ErrorCodeChannelRateLimitExceeded)
	}
	if !common.CheckRPM("channel", channel.Id, chSetting.Rpm) {
		return types.NewError(errors.New("channel RPM limit exceeded"), types.ErrorCodeChannelRateLimitExceeded)
	}
