...
type RelayInfo struct {
	...
	// CapturedResponseText accumulates the assistant's response text for session
	// logging. For streaming responses the per-chunk handlers append the aggregated
	// text here (since the upstream body is consumed chunk-by-chunk); for non-stream
	// responses the handler stores the full response body. Empty when session
	// logging is disabled. Consumed at PostTextConsumeQuota.
	CapturedResponseText string
